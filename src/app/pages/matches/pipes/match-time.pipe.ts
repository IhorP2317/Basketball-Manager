...
export class MatchTimePipe implements PipeTransform {
  ...
  transform(match: Match | null | undefined): string {
    if(match == null){
      return '';
    }
    ...
    const formattedStartTime = this.datePipe.transform(startDateTime, 'dd/MM/yyyy HH:mm:ss');
    if (!endDateTime) {
      return formattedStartTime || '';
    }

    // Format the end time if it exists
    const formattedEndTime = this.datePipe.transform(endDateTime, 'dd/MM/yyyy HH:mm:ss');
